...
public function createBulkFromCsv($filePath) {
    ...
    // Helper function to get course ID from code (this part is correct)
    $courseCache = [];
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        if (empty($fullName) || empty($email) || empty($courseCode)) {
            $failures[] = "Row $rowNumber: Full name, email, and course code are required.";
            continue;
        }
        ...
        try {
            $existingStudent = $this->studentModel->findByEmail($email); // Use the studentModel

            if ($existingStudent) {
                // SCENARIO 2: STUDENT EXISTS
                $studentId = $existingStudent['id'];
                if ($this->studentModel->isEnrolled($studentId, $courseId)) {
                    // Student is already in this specific course, this is a "soft" failure.
                    $failures[] = "Row $rowNumber ($email): Already enrolled in this course.";
                } else {
                    // Student exists but is enrolling in a NEW course.
                    $this->studentModel->enrollInCourse($studentId, $courseId);
                    $successCount++;
                }
            } else {
                // SCENARIO 1: NEW STUDENT
                // The password is only required if the student is new.
                if (empty($password)) {
                    $failures[] = "Row $rowNumber ($email): Password is required for new students.";
                    continue;
                }
                $this->studentModel->register($fullName, $email, $password, $courseId); // Use the register method
                $successCount++;
            }
        } catch (Exception $e) {
            $failures[] = "Row $rowNumber ($email): A database error occurred - " . $e->getMessage();
        }
    ...
    if (!empty($failures) && $successCount === 0) {
        // Only roll back if absolutely nothing succeeded.
        $this->db->rollBack();
        return ['success' => 0, 'failures' => $failures, 'error' => 'Import failed due to errors. No users were added.'];
    } else {
        // Commit the transaction. This will save all successful new students AND new enrollments.
        $this->db->commit();
