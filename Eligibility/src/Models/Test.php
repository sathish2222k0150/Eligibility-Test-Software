...
class Test {
    ...
    public function getAvailableForStudent($studentId) {
        ...
        $sql = "
            ...
            -- Only select tests where the course_id is in the list of courses the student is enrolled in
            WHERE t.course_id IN (
                SELECT course_id FROM student_courses WHERE user_id = ?
            )
            AND t.status = 'published' 
            -- And the student has not already attempted this test
            AND t.id NOT IN (
                SELECT test_id FROM student_test_attempts WHERE student_id = ?
            )
        ";
        $stmt = $this->db->prepare($sql);
        // We need to bind the studentId twice for the two subqueries
